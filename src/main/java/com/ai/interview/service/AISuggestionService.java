...
public class AISuggestionService {
    ...
    private String buildWrongAnswerText(List<WrongSubmitWithQuestionVO> wrongSubmits) {
        if (wrongSubmits.isEmpty()){
            return "该分类暂无错题记录，暂时无法生成建议。";
        }
        StringBuilder sb = new StringBuilder();
        for (WrongSubmitWithQuestionVO item : wrongSubmits){
            sb.append("题目：").append(item.getTitle()).append("\n");
            sb.append("内容：").append(item.getContent()).append("\n");
            sb.append("正确答案：").append(item.getCorrectOptionLabel()).append("\n");
            sb.append("你选的答案：").append(item.getSelectedOptionLabel()).append("\n");
            sb.append("解析：").append(item.getAnalysis()).append("\n");
        }
        return sb.toString();
    }
}
